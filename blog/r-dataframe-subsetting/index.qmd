...
It is easy to shoot yourself in the foot with partial matching. Therefore, I strongly advise against using the `$` notation when working with `data.frame` objects.
...
Both `data.frame` and `tibble` objects both return the desired column as a vector.
