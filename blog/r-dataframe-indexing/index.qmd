...
Selecting a specific column is an important operation in almost any data analysis pipeline. In the following examples, we will explore different options to select the second column (named `"second"`).
...
We can enclose the desired column name in quotes, but the first variant without quotes is more common. In either case, R returns the single column as a basic vector. This is also true when using a `tibble`:
...
:::{.callout-important}
Indexing a `data.frame` with `$` performs partial matching. This means that R will return the first column that partially matches the given name, for example:
...
:::{.callout-tip}
The `$` operator applied to a `tibble` does *not* perform partial matching, so the following example will result in `NULL` and raise a warning:
...
It is very easy to shoot yourself in the foot with partial matching, especially if you make a typo. Therefore, I advise against using the `$` notation when working with `data.frame` objects. There are plenty of alternatives available, which we will discuss next.
