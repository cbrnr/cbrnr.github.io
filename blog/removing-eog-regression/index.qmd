...
::: {.callout-note}
MNE now provides a built-in method for removing artifacts via regression (check out the [tutorial](https://mne.tools/stable/auto_tutorials/preprocessing/35_artifact_correction_regression.html) for more information). However, since the method is rather simple, it is also instructive to implement it manually, which is what we will do in this post.
:::
...
We are interested in EEG data from the calibration run (third run, list index `2`) and the experimental run (fourth run, list index `3`), which we name `eeg1` and `eeg2` (note that we also multiply by 10<sup>-6</sup> to scale the signals from microvolts to volts). This results in two NumPy arrays:
